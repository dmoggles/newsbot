...
class BlueSkyPoster:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if self.handle and self.app_password:
            try:
                ...
                self.logger.error(f"Failed to initialize BlueSky client: {e}")
                self.client = None
                self.authenticated = False
        else:
            self.client = None
        ...
        self.logger.info(f"Initialized BlueSkyPoster with handle: {self.handle}, "
                        f"post_interval: {self.post_interval_minutes} minutes")
    ...
    def authenticate(self) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Failed to authenticate with BlueSky: {e}")
            self.authenticated = False
            return False
        ...
        try:
            ...
            if not url:
                ...
                try: 
                    ...
                    self.logger.error(f"Failed to create web card for URL {url}: {e}")
                    post_obj = Post(post_text)
            ...
            self.logger.info(f"Successfully posted story {story.story_id} to BlueSky")
            return PostStatus.posted, f"Posted successfully at {self.last_successful_post_time}"
            
        except Exception as e:
            self.logger.error(f"Failed to post story {story.story_id} to BlueSky: {e}")
    ...
    def post_stories(self, stories: list[Story]) -> Dict[str, Any]:
        ...
        for story in stories:
            ...
            if story.post_status == PostStatus.posted:
                stats["skipped"] += 1
                self.logger.info(f"Skipping story {story.story_id} - already posted")
                continue
        ...
        self.logger.info(f"Posting complete: {stats}")
        return stats
    ...
    def set_last_post_time(self, last_post_time: datetime):
        ...
        self.logger.info(f"Set last successful post time to: {last_post_time}")
